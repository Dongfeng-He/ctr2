...
if __name__ == "__main__":
    ...
    # 修正时间的位数
    answer_df["time"] = answer_df["time"].parallel_apply(fix_time)
    invite_df["time"] = invite_df["time"].parallel_apply(fix_time)
    # 添加类型，回答是0，邀请是1
    ...
    # 问题记录按时间排序
    member_record["answer_list"] = member_record["answer_list"].parallel_apply(lambda x: sorted(x, key=lambda y: y[1]))

    """
    处理 question_df
    """
    # 标题、内容 str 解析成 list
    question_df['title_single_words'] = question_df['title_single_words'].parallel_apply(parse_list_2)
    question_df['title_words'] = question_df['title_words'].parallel_apply(parse_list_1)
    question_df['content_single_words'] = question_df['content_single_words'].parallel_apply(parse_list_2)
    question_df['content_words'] = question_df['content_words'].parallel_apply(parse_list_1)
    question_df['topics'] = question_df['topics'].parallel_apply(parse_list_1)
    # title、desc 词计数，topic 计数
    question_df['num_title_words'] = question_df['title_single_words'].parallel_apply(len)
    question_df['num_desc_words'] = question_df['content_single_words'].parallel_apply(len)
    question_df['num_topics'] = question_df['topics'].parallel_apply(len)

    """
    处理 member_df
    """
    # 将 topic 解析成 list 和 dict
    member_df['subscribe_topics'] = member_df['subscribe_topics'].parallel_apply(parse_list_1)
    member_df['interest_topics'] = member_df['interest_topics'].parallel_apply(parse_map)
    # 用户关注和感兴趣的 topic 数
    member_df['num_subscribe_topics'] = member_df['subscribe_topics'].parallel_apply(len)
    member_df['num_interest_topics'] = member_df['interest_topics'].parallel_apply(len)  # 人工计算，上限为10
